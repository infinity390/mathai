...
def solve_graphics(Ax, Ay, Az, Bx, By, Bz, BAx, BAy, BAz, phi, theta, fvl, tvl, length):
    ...
    while done:
        ...
        for i in range(len(q.children)):
            ...
            # Apply trig identities during simplification
            item = fraction(expand(trig0(fx(simplify(item)))))
            q.children[i] = item
            
            # Solvable equation check
            vars_in_item = vlist(item)
            if len(vars_in_item) == 1:
                target_var = vars_in_item[0]
                # Pass the full equation node (or LHS - RHS) so inverse() has both sides
                solution[target_var] = inverse(item.children[0], target_var)
                done = True
                break
                
    # Map back to readable parameter names safely
    final_solution = {}
...
def solve_graphics_string(s):
    # Use s.split() to handle arbitrary whitespace cleanly
